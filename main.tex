...
\usepackage[default,defaultsans,scale=0.97]{opensans}  % Open Sans als Hauptschrift (auf ~2.200 Zeichen/Seite kalibriert, Leitfaden Kap. 5.2.2)
% Die Option "default" belegt nur \familydefault, NICHT \sfdefault. Ohne
% "defaultsans" greift jedes \sffamily -- in den Ueberschriften-Fonts unten
% und auf dem Titelblatt -- auf Computer Modern Sans zu und bringt damit
% eine zweite Schriftart ins Dokument (Verstoss gegen Leitfaden Kap. 5.2.2,
% "durchgaengig verwendet"). \ttdefault bleibt bewusst Inconsolata, weil
% Kap. 5.9.2 fuer Quelltexte ausdruecklich eine Fixtype verlangt.
...
% Schriftgrad 10 pt (Times New Roman) oder 9,5 pt (Arial)." Open Sans ist
% hier das Arial-Analogon, massgeblich sind also 9,5 pt.
% ACHTUNG: Der folgende \setkomafont-Aufruf bleibt aus Dokumentations-
% gruenden stehen, ist aber WIRKUNGSLOS -- footmisc ersetzt weiter unten
% \@footnotetext und umgeht den KOMA-Haken. Der tatsaechlich wirksame
% Schriftgrad wird direkt nach \usepackage{footmisc} gesetzt.
...
% Fließtext optisch (Ergebnis "1011" statt "10, 11").
% BEFUND 29.07.2026 (Glyph-Analyse des Kompilats): Der footmisc-Mechanismus
% (Option `multiple`, kernbasierte Folge-Erkennung ueber \FN@mf@check /
% \FN@mf@prepare) ist in diesem Dokument WIRKUNGSLOS, weil pdfx/hyperref
% (Ladereihenfolge: footmisc Z.~239, pdfx Z.~401) \@footnotemark anschliessend
% fuer die Hyperlink-Anker neu definiert und dabei die footmisc-Haken
% verwirft. Folge: \multfootsep wird niemals ausgewertet; zwischen den
% Marken steht lediglich ein Leerraum (-172/1000 em im Content-Stream,
% S.~17: "10 11"). Der Trenner wird deshalb weiter unten im
% Bibliografie-Abschnitt in der Neudefinition von \mkbibfootnote mit einem
% eigenen, hyperref-festen Marker-Verfahren umgesetzt (siehe dort).
% Das Paket footmisc selbst wird weiterhin benoetigt (u. a. ersetzt es
% \@footnotetext, worauf der Schriftgrad-Patch unten aufbaut).
\usepackage[multiple]{footmisc}

% --- Fussnoten-Schriftgrad (Leitfaden Kap. 5.2.2) ---------------------------
% BEFUND 25.07.2026: footmisc ersetzt \@footnotetext und umgeht dabei den
% KOMA-Font-Haken \setkomafont{footnote} (Zeile oben). Der Schriftgrad kam
% deshalb faktisch aus \footnotesize (= 9 pt bei 11-pt-Grundschrift) und lag
% damit UNTER den vom Leitfaden genannten Werten -- die 10-pt-Angabe im
% \setkomafont-Aufruf war wirkungslos. Nachgemessen im Kompilat: 8,73 pt
% physisch (9 pt nominal x opensans-scale 0,97).
% Leitfaden Kap. 5.2.2 fordert 10 pt (Times New Roman) bzw. 9,5 pt (Arial);
% da Open Sans hier als Arial-Analogon gefuehrt wird, gilt der 9,5-pt-Wert.
% Der Grad wird daher NACH footmisc erzwungen. Zeilenabstand 11,5 pt = 1,21x
% und damit einzeilig im Sinne des Leitfadens.
\makeatletter
\newcommand*{\FOM@fnsize}{\fontsize{9.5pt}{11.5pt}\selectfont}
\let\FOM@orig@footnotetext\@footnotetext
% \footnotesize wird innerhalb von \@footnotetext gesetzt, BEVOR \@makefntext
% die Fussnotenziffer ausgibt. Der Grad wird deshalb ueber ein lokales \let
% umgehaengt -- so tragen Ziffer UND Text denselben Grad (ein Patch, der nur
% das Argument #1 umschliesst, laesst die Ziffer auf 9 pt zurueck).
\long\def\@footnotetext#1{%
    \begingroup
      \let\footnotesize\FOM@fnsize
      \FOM@orig@footnotetext{#1}%
    \endgroup}
\makeatother
% Abstand ZWISCHEN zwei Fussnoten muss groesser sein als der Zeilenabstand
% INNERHALB einer Fussnote (11,5 pt), sonst verschmelzen mehrere Fussnoten
% optisch zu einem Block.
\setlength{\footnotesep}{13pt}

% URLs in der Schrift der Umgebung setzen statt in der CM-Ersatzfamilie:
% Computer Modern kennt den Grad 9,5 pt nicht und faellt auf 9 pt zurueck --
% ohne diese Zeile laufen URLs in Fussnoten sichtbar kleiner und in einer
% anderen Schriftfamilie als der uebrige Fussnotentext. \urlstyle stammt aus
% url, das erst ueber pdfx/hyperref geladen wird, daher \AtBeginDocument.
\AtBeginDocument{\urlstyle{same}}
% HINWEIS 29.07.2026: Eine fruehere Umdefinition von \multfootsep an
% dieser Stelle wurde entfernt -- sie war wirkungslos, weil der
% footmisc-Trennermechanismus unter pdfx/hyperref nicht greift
% (siehe Kommentarblock oben). Der Trenner wird im \mkbibfootnote-Patch
% weiter unten selbst umgesetzt.
...
\svgpath{{abbildungen/}}
\svgsetup{inkscapelatex=false}   % Text von Inkscape direkt ins PDF einbetten (verhindert ueberlagerte Text-Artefakte in den Architektur- und Strukturgrafiken)
...
% Float-Platzierung (31.07.2026): Grosse Abbildungen (v. a. Abbildung 3.1,
% ~71 %% der Texthoehe) landeten mit den LaTeX-Defaults (\topfraction=0.7,
% \floatpagefraction=0.5) auf einer isolierten Gleitseite ohne Fliesstext.
% Die angehobenen Grenzen erlauben die Platzierung am Seitenkopf MIT Text
% darunter und verlangen fuer eine reine Gleitseite mindestens 80 %% Fuellung;
% die Lesbarkeit bleibt unveraendert, da die Grafikgroesse nicht reduziert wird.
...
%   \caption[Kurztitel]{Langtitel.\quelleeigenki{Anbieter Modell (Anbieter, Jahr)}}
%                                                    % KI-erstellte Darstellung;
%                                                    % Kurzbeleg zwingend
...
% Leitfaden Kap. 6.2: "Alle verwendeten Quellen ... sind zu referenzieren --
% im Umkehrschluss impliziert eine fehlende Quelle, dass die Passage von der
% Autorin oder dem Autor stammt; dies muss nicht explizit angegeben werden.
% Insbesondere sollte auf den Vermerk \enquote{Eigene Darstellung} verzichtet
% werden." Das Makro gibt daher nichts mehr aus; die Aufrufe bleiben an den
% Tabellen stehen, damit die Entscheidung an EINER Stelle umkehrbar ist.
\newcommand{\quelleeigen}{}
% KI-Kennzeichnung nach Leitfaden Kap. 6.2.4: Anbieter und Jahr der Abfrage
% analog zum Kurzbeleg. OHNE den Zusatz \enquote{Eigene Darstellung} -- den
% untersagt Kap. 6.2 ausdruecklich (siehe \quelleeigen oben).
\newcommand{\quelleeigenki}[1]{\par\vspace{2pt}\raggedright\small Erstellt mit #1.}
...
% Trennzeichen bei Mehrfachnachweisen (\footcites{a}{b}{c}) explizit
% festgeschrieben. biblatex-Standard ist das Semikolon; die Angabe hier
% macht das Trennzeichen unabhaengig von Stil-Updates deterministisch.
% (Leitfaden Kap. 6: "Zitationen sind einheitlich zu gestalten" --
% Semikolon erfuellt die Vorgabe; verifiziert im Kompilat V1.0, S. 13,
% Fussnote 1: "...ISBN: 978-0-13-461099-3; vgl. B. Gollnick, ...")
...
% UEBERARBEITET 29.07.2026 nach Glyph-Analyse des Kompilats:
% Der footmisc-Trennermechanismus (Option `multiple`) greift in diesem
% Dokument nicht, weil pdfx/hyperref \@footnotemark nach footmisc neu
% definiert und dessen Kern-Markierung (\FN@mf@prepare/\FN@mf@check)
% verwirft; \multfootsep wird dadurch nie ausgewertet. Im Kompilat stand
% zwischen aufeinanderfolgenden Marken lediglich ein Leerraum
% ("10 11" statt "10, 11").
% Die Neudefinition von \mkbibfootnote setzt den Trenner deshalb mit
% einem eigenen Marker-Verfahren um, das dem footmisc-Prinzip folgt,
% aber vollstaendig in unserer Kontrolle liegt:
%   * \FOM@fnmark@prepare hinterlaesst hinter JEDER Fussnoten-Marke ein
%     sich aufhebendes Kern-Paar (letzter Listenknoten: Kern 7sp).
%   * \FOM@fnmark@check prueft zu Beginn der NAECHSTEN Fussnote, ob der
%     letzte Knoten dieser Marker ist; nur dann wird vor der neuen Marke
%     der Trenner \FOM@fnmark@sep gesetzt.
%   * Das vorangestellte \unskip entfernt Zwischenraum vor der Marke
%     (u. a. den im Kompilat beobachteten Leerraum zwischen den Marken)
%     und legt den Marker frei; ohne Marker wirkt \unskip wie das
%     klassische biblatex-\unspace (Marke schliesst an das Wort an).
% Da biblatex-chicago Fussnoten-Citationen ausschliesslich ueber
% \mkbibfootnote abwickelt, wirkt der Patch fuer \footcite, \footcites
% und \autocite. Der Wert 7sp kollidiert nicht mit dem (hier toten)
% footmisc-Marker 3sp. Satzzeichen vor der Folgefussnote bleiben
% unberuehrt, weil dann kein Marker am Listenende steht.
\makeatletter
\newcommand*{\FOM@fnmark@sep}{,\space}%  % sichtbarer Trenner: "10, 11"
\newcommand*{\FOM@fnmark@prepare}{\ifhmode\kern-7sp\kern7sp\relax\fi}
\newcommand*{\FOM@fnmark@check}{%
    \ifhmode
        \unskip
        \ifdim\lastkern=7sp\relax
            \unkern
            \textsuperscript{\FOM@fnmark@sep}%
        \fi
    \fi}
...
% Wrapper fuer MANUELLE \footnote: Damit auch sie am Trennerverfahren
% teilnimmt (Marker pruefen + Marker hinterlassen). Befund 29.07.2026:
% In Kap. 2 folgt auf eine \footcites-Gruppe (Marke 42) eine manuelle
% \footnote (Marke 43) -- ohne Wrapper stand dort weiterhin "42 43".
\newcommand*{\FOMfootnote}[1]{%
    \FOM@fnmark@check\footnote{#1}\FOM@fnmark@prepare}
\makeatother
...
% Hilfsmakros für Fußnoten-Belege -- AUSSER BETRIEB seit 29.07.2026.
% Der Fließtext nutzt durchgaengig die biblatex-Standardbefehle:
%   \footcite[vgl.][S.~24]{key}              -> Fußnote mit "Vgl. ..."
%   \footcite[][S.~24]{key}                  -> ohne "Vgl." (woertliches Zitat)
%   \footcites[vgl.][S.~24]{a}[vgl.][]{b}    -> mehrere Belege in EINER Fußnote
% Begruendung der Umstellung: Die Wrapper kapselten nur \footcite und konnten
% die Mehrfachsyntax von \footcites nicht abbilden; Kapitel 1 und 2 brauchen
% sie. Ein Sonderbefehl fuer etwas, das biblatex bereits standardisiert
% anbietet, ist zudem eine unnoetige Fehlerquelle bei der Uebergabe.
% Die Definitionen bleiben nur als Notnagel auskommentiert stehen, falls ein
% Altbestand nachgezogen werden muss. Beim Wiedereinschalten pruefen, ob im
% Text \footcites-Aufrufe stehen -- die Wrapper ersetzen sie NICHT.
...
\newcommand{\arbeitUntertitel}{Sicherheitsorientiert begrenzte KI-Autonomie im automatisierten Derivatehandel}
...
%   - Tabellenverzeichnis: seit V1_8 AKTIV (verzeichnisse/tabellenverzeichnis.tex);
%     bis V1_7 ausgeblendet, da die Arbeit keine Tabellen enthielt. Mit den
%     Anhaengen "Evaluationsbelege" (4 Tabellen) und "Projektstruktur und
%     Quellcodearchiv" (4 Tabellen) gemaess Leitfaden Kap. 2.4 / 5.1 erforderlich.
...
% Reihenfolge der Vorspann-Verzeichnisse: ALPHABETISCH (Abbildungs- ->
% Abkuerzungs- -> Formelzeichen- -> Glossar -> Tabellenverzeichnis).
% BEWUSSTE PROJEKTFESTLEGUNG, abweichend vom Leitfaden Kap. 5.1, S. 12:
% Der Leitfaden listet dort Inhalt -> Abbildung -> Tabelle -> Abkuerzung
% -> Glossar -> Formelzeichen -> Quelltext. Diese Liste ist im Leitfaden
% nicht ausdruecklich als verbindliche Reihenfolge gekennzeichnet; die
% alphabetische Sortierung wird als einheitliches, nachvollziehbares
% Ordnungsprinzip beibehalten (Entscheidung Kiss, 25.07.2026).
\input{verzeichnisse/abbildungsverzeichnis}         % Kap. 2.3 / 5.1 / 5.7
\input{verzeichnisse/abkuerzungen}                  % Kap. 2.5 / 5.1
\input{verzeichnisse/formelzeichen}                 % Kap. 5.1 / 5.8
\input{verzeichnisse/glossar}                       % Kap. 5.1
\input{verzeichnisse/tabellenverzeichnis}           % Kap. 2.4 / 5.1
\clearpage

% Quelltextverzeichnis bewusst nicht aktiviert. Begruendung (Stand V1_8):
%   - Leitfaden Kap. 5.1 stuft das Verzeichnis als optional ein
%     ("kann sinnvoll sein, wenn ...").
%   - Seit dem Wegfall des Quelltext-Abdrucks (Anhang I = Quellcodearchiv,
%     Quellcode liegt als Zip-Beilage vor) enthaelt die Arbeit nur noch EIN
%     Listing (Backtest-Protokoll in Anhang II, bewusst ohne \caption) —
%     \lstlistoflistings waere damit LEER.
% Bei Bedarf — etwa wenn kuenftig betitelte Listings eingesetzt werden —
% die folgenden Zeilen aktivieren (KOMA fuegt mit listof=totoc automatisch
% einen TOC-Eintrag hinzu, daher KEIN \addcontentsline noetig):
%\lstlistoflistings
%\cleardoublepage
% OFFEN FUER DEN FINALEN REVIEW (vermerkt 25.07.2026, zurueckgestellt):
% Leitfaden Kap. 5.9.2 sieht vor, dass bei selbst entwickelter Software
% die wesentlichen Quelltextpassagen wiedergegeben und ueber Titel in ein
% Quelltextverzeichnis aufgenommen werden. Derzeit enthaelt die Arbeit
% keinen betitelten eigenen Quelltext. Vor der Abgabe ist zu entscheiden,
% ob ein kurzes Listing (z. B. Regimebestimmung oder Aggregation der
% Modellvoten, 10--15 Zeilen) aufgenommen und das Verzeichnis oben
% aktiviert wird. Gegenargument: Kap. 5.1 S. 14 und Kap. 7.3 lassen die
% Auslagerung umfangreichen Materials auf den Datentraeger ausdruecklich
% zu -- die bestehende Archivloesung ist insofern leitfadenkonform.
...
\include{kapitel/03_umsetzung}
\include{kapitel/04_schluss}
...
\cleardoublepage
\input{verzeichnisse/ki-verzeichnis}
...
% ToC-Umbruch vor dem Anhang: deaktiviert. Der erzwungene Umbruch stammt aus
% einem Stand mit kuerzerem Inhaltsverzeichnis; inzwischen fuehrt er zu einer
% sehr leeren ersten ToC-Seite. Zum Reaktivieren die folgende Zeile
% einkommentieren.
...
\include{anhang/eigenstaendigkeitserklaerung}
