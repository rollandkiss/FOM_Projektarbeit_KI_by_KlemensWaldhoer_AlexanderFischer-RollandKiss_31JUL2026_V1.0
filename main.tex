...
% Hinweis: Die Kopfzeilen-Marke der Kapitel wird ueber \automark (oben)
% gesetzt. Das mehrseitige Symbolverzeichnis (langer longtable mit vielen
% Beleg-Fussnoten) erhielt darueber zuverlaessig die falsche Marke des
% Vorkapitels. Es verwendet deshalb einen markenUNABHAENGIGEN, festen
% Seitenstil \symbolkopf (siehe unten, nach der Kopfzeilen-Definition).
...
% ----------------------------- Fester Seitenstil: Symbolverzeichnis ---------
% Markenunabhaengige Kopfzeile fuer das mehrseitige Symbolverzeichnis. Der
% Kopf zeigt fest "Symbolverzeichnis" (kein \headmark), sodass weder
% longtable noch der Beleg-/Glossar-Apparat die Marke verfaelschen koennen.
% Layout identisch zu den uebrigen Kopfzeilen (Akzentlinie/Seitenzahl).
\newpairofpagestyles{symbolkopf}{%
    \ihead{%
        \begin{tabular}[b]{@{}l@{}}%
            Symbolverzeichnis \\[-2pt]%
            \textcolor{FOMakzent}{\rule{\akzentlinienBreiteHaupt}{0.8pt}} \\
        \end{tabular}%
    }%
    \ohead*{%
...
% pdflscape rotiert einzelne Seiten im PDF (anstatt nur den Satzspiegel).
% Damit lassen sich breite Diagramme (AR >= 2) lesbar darstellen, ohne den
% Lesefluss der uebrigen Seiten zu beeintraechtigen (FOM-Leitfaden Kap. 5.7:
% Lesbarkeit der Abbildung).
\usepackage{pdflscape}
% TikZ fuer das Symbolverzeichnis (BPMN-2.0-Notationselemente).
% Die Symbole werden direkt im LaTeX-Quelltext gezeichnet, damit sie
% ohne externe Bilddateien aufloesungsunabhaengig im PDF/A-Build
% (pdfx, a-2b) verfuegbar bleiben. Geladene Bibliotheken:
%   - arrows.meta   : moderne Pfeilspitzen (Latex/Stealth) fuer Sequenz-
%                     und Nachrichtenflusssymbole
%   - shapes.geometric : Diamant-Form fuer Gateways
%   - positioning   : einheitliche relative Positionierung kleiner Marker
\usepackage{tikz}
...
% ----------------------------- Algorithmen / Quelltexte ---------------------
% Leitfaden Kap. 5.9.1 (Algorithmen / Pseudocode): Titel, fortlaufende
% Nummerierung sowie nummerierte Zeilen sind verbindlich.
\usepackage[ruled,linesnumbered,vlined]{algorithm2e}
\SetAlgorithmName{Algorithmus}{Algorithmus}{Algorithmenverzeichnis}
\SetAlgoCaptionSeparator{:}
% algorithm2e platziert die Zeilennummern bei aktivem `linesnumbered`-Modus
% am linken Innenrand der Algorithmus-Box. In der Standardkonfiguration
% reicht der voreingestellte `algomargin` (~2 em) nicht aus, sodass die
% Zeilennummern links aus den horizontalen Begrenzungslinien austreten und
% in den linken Seitenrand ragen. Mit \IncMargin wird der Innenrand
% vergroessert, sodass die Zeilennummern bündig innerhalb der Box stehen
% und der Satzspiegel nicht verletzt wird.
\IncMargin{2em}
% algorithm2e setzt den Page-Mark fuer das Algorithmenverzeichnis intern via
% \MakeUppercase, wodurch der Kopf "ALGORITHMENVERZEICHNIS" zeigt. Wir
% ueberschreiben \listofalgorithms, damit der Mark in normaler Gross-Klein-
% Schreibung gesetzt wird (Vereinheitlichung mit den anderen Verzeichnissen).
\makeatletter
...
% Eintraege im Algorithmenverzeichnis flush-left setzen (analog zu
% Abbildungs- und Tabellenverzeichnis -- konsistent mit dem
% FOM-Leitfaden-Layout).
\renewcommand*{\l@algocf}[2]{\@dottedtocline{1}{0pt}{2.5em}{#1}{#2}}
\makeatother
...
% Eigener Float-Typ `mathformel` (siehe \DeclareNewTOC weiter unten):
% Ohne diese Registrierung liefert \Cref{mformel:...} im PDF "??".
\crefname{mathformel}{Formel}{Formeln}
\Crefname{mathformel}{Formel}{Formeln}
...
% ----------------------------- Mathematisches Formelverzeichnis ------------
% Eigenes Float-Verzeichnis fuer mathematische Formeln (separates
% Verzeichnis neben dem von Kap. 5.1 angesprochenen "Verzeichnis von
% Formelzeichen und Indizes"). Verwendung im Kapitel:
%
%   \begin{mathformel}
%       \caption{Anlagenverfuegbarkeit}\label{eq:verfuegbarkeit}
%       \[ a = \frac{t_B}{t_{\text{ges}}} \]
%   \end{mathformel}
%
% Numerierung pro Kapitel (Formel 2.1, 2.2, ...). Eintrag im Inhalts-
% verzeichnis ueber listof=totoc bereits aktiv.
\usepackage{float}
\DeclareNewTOC[%
    type=mathformel,%
    types=mathformeln,%
    ...
    floatpos=H,%
    name=Formel,%
    listname={Mathematisches Formelverzeichnis}%
]{lomf}
\numberwithin{mathformel}{chapter}
\DeclareTOCStyleEntry[indent=0pt,numwidth=3em]{tocline}{mathformel}
...
\newcommand{\autorVorname}{Rolland}
\newcommand{\autorNachname}{Kiss}
\newcommand{\autorMatrikel}{799553}
...
% Sperrvermerk (Leitfaden Kap. 7.2). Bei NICHT vertraulichen Arbeiten
...
%   Abbildungs- -> Abkuerzungs- -> Glossar -> Symbolverzeichnis.
...
%     (die \begin{mathformel}-Floats im Hauptteil bleiben gueltig
%     nummeriert, werden aber nicht mehr in einem Verzeichnis gelistet).
...
\input{verzeichnisse/symbolverzeichnis}             % BPMN-2.0-Notationssymbole
